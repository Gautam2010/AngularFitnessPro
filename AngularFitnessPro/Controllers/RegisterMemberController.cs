...
using AngularFitnessPro.Common;
using AngularFitnessPro.Interface;
using AngularFitnessPro.Models;
using AngularFitnessPro.ViewModels;

namespace AngularFitnessPro.Controllers
{
    [Authorize]
    ...
    public class RegisterMemberController : ControllerBase
    {
        ...
        public RegisterMemberController(IUrlHelper urlHelper, IMemberRegistration memberRegistration, IMapper mapper)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!_memberRegistration.CheckNameExits(member.MemberFName, member.MemberLName, member.MemberMName))
                    {
                        ...
                        automember.MemberNo = string.Format("{0}", DateTime.Now.Ticks);
